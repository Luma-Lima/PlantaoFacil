...
angular
  .module('mwl.calendar')
  .controller('MwlDateModifierCtrl', ["$element", "$attrs", "$scope", "moment", "paintService", function($element, $attrs, $scope, moment, paintService) {
    ...
    function onClick() {
      ...
      $scope.momentNewDate = moment(vm.date);
      $scope.momentNewDate.set("date", 1);

      vm.referenceDate = moment("2017-10-01")

      paintService.setMomentDate($scope.momentNewDate);
      paintService.setAlreadyPainted(false);
...
angular
  .module('mwl.calendar')
  .controller('MwlDynamicDirectiveTemplateCtrl', ["$compile", "$scope", "$attrs", "$element", "$templateCache", "$log", "calendarConfig", "paintService", function($compile, $scope, $attrs, $element, $templateCache, $log, calendarConfig, paintService) {

    $scope.paintService = paintService;

    $scope.$watch($attrs.overrides, function(overrides) {

      if(paintService.getPaintedStatus() === false) {
        paintService.paintWeeksDynamically(paintService.getReferenceDate(), paintService.getMomentDate());
      }
      paintService.setAlreadyPainted(true);
